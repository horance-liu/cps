...
当仅仅对\ascii{amountInBaseUnit}声明产生依赖时，前置声明即可，无需包含整个\ascii{Length}的头文件。另外，如果\ascii{Quantity}存在\ascii{virtual}函数时，\ascii{Length}还有进一步扩展\ascii{Quantity}的可能性，从而使设计提供了更大的灵活性。
